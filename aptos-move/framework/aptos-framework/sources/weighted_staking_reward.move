...
module aptos_framework::weighted_staking_reward {
    ...
    public entry fun initialize(
        ...
        for (i in 0..n) {
            buckets.push_back(
                BucketConfig { duration_secs: durations_secs[i], multiplier_bps: multipliers_bps[i] }
            );
        };

        move_to(aptos_framework, BonusConfig { base_share_bps: DEFAULT_BASE_SHARE_BPS, buckets });
    ...
    public entry fun update_base_share(aptos_framework: &signer, new_base_share_bps: u64) {
        system_addresses::assert_aptos_framework(aptos_framework);
        assert!(
    ...
    inline fun borrow_config(): &BonusConfig {
        assert!(exists<BonusConfig>(@aptos_framework), error::not_found(ECONFIG_NOT_INITIALIZED));
    ...
    inline fun get_bucket(bucket_id: u64): BucketConfig {
        ...
        assert!(bucket_id < config.buckets.length(), error::invalid_argument(EINVALID_BUCKET_ID));
    ...
    inline fun calculate_pending_bonus_internal(
        ...
        if (earned_total > position.debt) { ((earned_total - position.debt) as u64) } else { 0 }
    }
    ...
    public fun get_total_bucket_shares(pool_address: address, user: address): u128 {
        if (!exists<BonusPoolState>(pool_address)) { return 0 };
        let bonus_pool = &BonusPoolState[pool_address];
        if (!bonus_pool.positions.contains(&user)) { return 0 };
        bonus_pool.positions.borrow(&user).shares
    }
    ...
    public fun get_position(pool_address: address, user: address): (u64, u128, u64, u64, u64) {
        ...
        let bucket = get_bucket(position.bucket_id);
        let (complete_cycles, _) = calculate_cycles(position.lock_start_secs, now, bucket.duration_secs);
    ...
    public(friend) fun split_and_deposit_rewards(
        ...
        if (!exists<BonusPoolState>(pool_address) || !exists<BonusConfig>(@aptos_framework)) {
            return reward_coins
        };
        ...
        if (bonus_amount == 0) { return reward_coins };
        ...
        bonus_pool.acc_bonus_per_weighted_share +=
            (bonus_amount as u128) * ACCUMULATOR_SCALE / bonus_pool.total_weighted_shares;
    ...
    public(friend) fun extract_bonus_payment(pool_address: address, amount: u64): Coin<AptosCoin> {
        assert!(
            exists<BonusPoolState>(pool_address),
            error::not_found(EBONUS_POOL_NOT_INITIALIZED)
        );
    ...
    /// shares: normalized delegation pool shares = raw_pool_shares / SHARES_SCALING_FACTOR
    ///         (≈ coin amount in octas at join time; safe scale for u128 accumulator math)
    public fun join_bucket(
        user: &signer, pool_address: address, bucket_id: u64, shares: u128
    ) {
        ...
        let bucket = get_bucket(bucket_id);
        let ws = weighted_shares(shares, bucket.multiplier_bps);
        let now = timestamp::now_seconds();
        ...
        bonus_pool.positions.add(
            user_addr, Position { bucket_id, shares, debt, lock_start_secs: now }
        );
        bonus_pool.total_weighted_shares += ws;

        event::emit(JoinedBucket { pool_address, user: user_addr, bucket_id, shares, lock_start_secs: now });
    }
    ...
    public fun upgrade_bucket(user: &signer, pool_address: address, new_bucket_id: u64): u64 {
        let user_addr = signer::address_of(user);
        assert!(
            exists<BonusPoolState>(pool_address),
            error::not_found(EBONUS_POOL_NOT_INITIALIZED)
        ...
        let (claimable, new_lock_start) =
            if (complete_cycles == 0 || elapsed == 0 || total_pending == 0) {
                ...
                let complete_secs = (complete_cycles as u128) * (old_bucket.duration_secs as u128);
                let c = ((total_pending as u128) * complete_secs / (elapsed as u128)) as u64;
                (c, old_pos.lock_start_secs + complete_cycles * old_bucket.duration_secs)
            };
        ...
        let new_debt = earned_at(new_weighted, bonus_pool.acc_bonus_per_weighted_share)
            - remaining_pending;
    ...
    public fun downgrade_bucket(
        ...
        let (claimed, burned, _) = exit_bucket(user, pool_address, shares);
        join_bucket(user, pool_address, new_bucket_id, shares);
        (claimed, burned)
    }
    ...
    public fun claim_bonus(user: &signer, pool_address: address): u64 {
        ...
        let claimable =
            if (complete_cycles == 0 || elapsed == 0 || total_pending == 0) { 0u64 }
                ...
                let complete_secs = (complete_cycles as u128) * (bucket.duration_secs as u128);
        ...
        pos.lock_start_secs = pos.lock_start_secs + complete_cycles * bucket.duration_secs;

        bonus_pool.positions.add(user_addr, pos);

        event::emit(
            BonusClaimed { pool_address, user: user_addr, bucket_id: pos.bucket_id, amount: claimable, complete_cycles }
        );
    ...
    public fun exit_bucket_all(user: &signer, pool_address: address): (u64, u64, u128) {
        ...
        exit_bucket(user, pool_address, shares)
    }
    ...
    /// Pass exit_shares == position.shares for a full exit (position is removed).
    /// Returns (bonus_claimed, bonus_burned, exit_shares)
    public fun exit_bucket(
        user: &signer, pool_address: address, exit_shares: u128
    ): (u64, u64, u128) {
        let user_addr = signer::address_of(user);
        assert!(
            exists<BonusPoolState>(pool_address),
        ...
        let (claimed, burned) =
            if (complete_cycles == 0 || elapsed == 0) { (0u64, exit_pending) }
            else {
                let complete_secs = (complete_cycles as u128) * (bucket.duration_secs as u128);
                let claimable = ((exit_pending as u128) * complete_secs / (elapsed as u128)) as u64;
