...
//   WritesAt   — modifies global<R>(a) → unchanged except at address a
...
module 0x42::frame_conditions {
    ...
    spec apply {
        pragma opaque;
        access_of<f>(a: address) reads Counter reads Config;
    ...
    spec apply_void {
        pragma opaque;
        access_of<f>(a: address) reads Config writes Balance;
    ...
    fun test_reads_frame(addr: address): u64 acquires Counter {
        apply(|a| get_counter(a) spec {
            ensures result == global<Counter>(a).value;
    ...
    spec test_reads_frame_wrong_result {
        pragma aborts_if_is_partial;
        ensures result == global<Counter>(addr).value + 1; // error: post-condition does not hold
    }
    ...
    spec test_writes_at_frame {
        ...
        ensures forall a: address where a != addr:
            global<Balance>(a) == old(global<Balance>(a));
    }
