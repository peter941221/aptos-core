...
module 0x42::write_and_return {
    ...
    spec write_and_read {
        ...
        ensures result == 42;
        ensures global<Counter>(addr).value == 42;
    }
    ...
    spec incr_and_read {
        pragma opaque;
        modifies global<Counter>(addr);
        ensures result == old(global<Counter>(addr).value) + 1;
    ...
    spec apply_rw {
        pragma opaque;
        modifies global<Counter>(x);
        access_of<f>(a: address) writes Counter;
    ...
    fun test_write_return(addr: address): u64 acquires Counter {
        apply_rw(|a| write_and_read(a) spec {
            ...
            ensures result == 42;
            ensures global<Counter>(a).value == 42;
        }, addr)
    }
    ...
    fun test_incr_return(addr: address): u64 acquires Counter {
        apply_rw(|a| incr_and_read(a) spec {
            modifies global<Counter>(a);
            ensures result == old(global<Counter>(a).value) + 1;
            ensures global<Counter>(a).value == old(global<Counter>(a).value) + 1;
            aborts_if !exists<Counter>(a);
            aborts_if global<Counter>(a).value + 1 > MAX_U64;
        }, addr)
    }
    ...
    spec test_incr_return_wrong {
        aborts_if !exists<Counter>(addr);
        aborts_if global<Counter>(addr).value + 1 > MAX_U64;
        ensures result == old(global<Counter>(addr).value); // error: post-condition does not hold
    }
}
