...
module 0x42::spec_functions {
    ...
    spec increment_if_active {
        ...
        ensures global<Config>(addr).active ==> counter_increased(addr);
    }
    ...
    spec apply {
        pragma opaque;
        access_of<f>(a: address) reads Config writes Counter;
    ...
    fun test_uses_old_in_closure(addr: address) acquires Counter, Config {
        apply(|a| increment_if_active(a) spec {
            ...
            ensures global<Config>(a).active ==> counter_increased(a);
        }, addr);
    }
    ...
    spec fun counter_non_negative(addr: address): bool {
        global<Counter>(addr).value >= 0  // always true for u64, but tests spec fun handling
    }
