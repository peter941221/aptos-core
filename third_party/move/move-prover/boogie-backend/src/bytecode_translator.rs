...
use move_model::{
    ast::{
        AccessSpecifierKind, AddressSpecifier, Attribute, BehaviorKind, ConditionKind, Exp,
        ExpData, FrameAccessKind, MemoryLabel, Operation as AstOperation, ResourceSpecifier,
        TempIndex, TraceKind, Value,
    },
...
impl FunctionTranslator<'_> {
    ...
    fn generate_function_body(&self) {
        ...
        self.declare_behavior_state_labels(fun_target, code, env, writer, &save_mem_set);
    ...
    /// Labels already declared by SaveMem bytecodes (in `save_mem_set`) are skipped.
    fn declare_behavior_state_labels(
        ...
        env: &GlobalEnv,
        writer: &CodeWriter,
        save_mem_set: &BTreeSet<(MemoryLabel, QualifiedInstId<StructId>)>,
    ) {
...
fn derive_param_frame_access(
    ...
    // Find access_of declaration for this parameter
    let param_access = fun_env.get_fun_param_access();
    let access_decl = param_access.iter().find(|a| a.fun_param == param_sym);

    match access_decl {
        None => {
            // No access_of declaration: conservatively assume all resources can be written
            for mem in used_memory {
                result.insert(mem, ApplyFrameAccess::WritesAll);
            }
        },
        Some(decl) => {
            // Classify each resource based on the access specifiers
            let mut write_resources: BTreeMap<QualifiedInstId<StructId>, Vec<String>> =
                BTreeMap::new();
            let mut read_resources: BTreeSet<QualifiedInstId<StructId>> = BTreeSet::new();

            for spec in &decl.specifiers {
                if let ResourceSpecifier::Resource(res) = &spec.resource.1 {
                    let res = res.clone().instantiate(&fun_qid.inst);
                    match spec.kind {
                        AccessSpecifierKind::Reads => {
                            read_resources.insert(res);
                        },
                        AccessSpecifierKind::Writes | AccessSpecifierKind::LegacyAcquires => {
                            // Check address specifier for WritesAt
                            let addr_str = match &spec.address.1 {
            ...
            for mem in used_memory {
                if let Some(addrs) = write_resources.get(&mem) {
                    ...
                    // Resource is in used_memory but not explicitly in access specifiers;
                    // conservatively treat as reads (since the parameter uses it but
                    // doesn't write it)
                    result.insert(mem, ApplyFrameAccess::Reads);
                }
            }
