...
//! Tests that `FunctionAttribute::ConstantAccessor` and `FunctionAttribute::Immutable`
//! are gated at the serializer/deserializer boundary for bytecode version 11.
//!
//! Both attributes are introduced at VERSION_11.  The serializer rejects them for earlier
...
//! either attribute cannot appear in a pre-V11 environment.
...
// ---------------------------------------------------------------------------
// FunctionAttribute::Immutable serde tests
// ---------------------------------------------------------------------------

/// Build a minimal module containing a public function with `FunctionAttribute::Immutable`
/// at the requested bytecode `version`.
fn make_module_with_immutable(version: u32) -> CompiledModule {
    let ident_self = Identifier::new("SELF").unwrap();
    let ident_locked = Identifier::new("locked").unwrap();
    ...
    CompiledModule {
        ...
        function_handles: vec![FunctionHandle {
            ...
            type_parameters: vec![],
            access_specifiers: None,
            attributes: vec![FunctionAttribute::Immutable],
        }],
        field_handles: vec![],
        friend_decls: vec![],
        struct_def_instantiations: vec![],
        ...
        field_instantiations: vec![],
        signatures: vec![sig_empty, sig_return],
        identifiers: vec![ident_self, ident_locked],
        address_identifiers: vec![addr],
        constant_pool: vec![],
        metadata: vec![],
        struct_defs: vec![],
        function_defs: vec![FunctionDefinition {
            ...
            code: Some(CodeUnit {
                locals: SignatureIndex(0),
                code: vec![Bytecode::LdU64(99), Bytecode::Ret],
            }),
        }],
        struct_variant_handles: vec![],
        struct_variant_instantiations: vec![],
...
/// Serializing a module with `FunctionAttribute::Immutable` at version 10 must fail
/// because the attribute was introduced at VERSION_11.
#[test]
fn test_immutable_attribute_not_serializable_at_v10() {
    let module = make_module_with_immutable(VERSION_10);
    let mut bytes = vec![];
    let result = module.serialize_for_version(Some(VERSION_10), &mut bytes);
    assert!(
        result.is_err(),
        "expected serialization to fail for FunctionAttribute::Immutable at version 10"
    );
}

/// A module carrying `FunctionAttribute::Immutable` serialized as V11 must be
/// rejected by a deserializer capped at V10, ensuring it cannot be loaded in a
/// pre-V11 environment.
#[test]
fn test_immutable_attribute_not_deserializable_at_v10() {
    let module = make_module_with_immutable(VERSION_11);
    let mut bytes = vec![];
    module
        .serialize_for_version(Some(VERSION_11), &mut bytes)
        .expect("V11 serialization should succeed");
    ...
    assert_eq!(
        result.unwrap_err().major_status(),
        StatusCode::UNKNOWN_VERSION,
        "expected UNKNOWN_VERSION: V11 module with Immutable cannot be loaded under V10 cap"
    );
}
