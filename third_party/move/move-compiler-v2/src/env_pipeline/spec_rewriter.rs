...
use move_model::{
    ast::{
        AccessSpecifier, AccessSpecifierKind, ConditionKind, Exp, ExpData,
        FunParamAccessSpecifiers, GlobalInvariant, MemoryRange, Operation, ResourceSpecifier,
...
pub fn run_spec_rewriter(env: &mut GlobalEnv) {
    ...
    for qid in spec_funs {
        ...
        let (final_usage, final_uses_old, final_old_memory) =
            if let Some(specifiers) = &decl.access_specifiers {
                let (spec_usage, spec_old_memory) =
                    derive_memory_from_access_specifiers(env, specifiers);
    ...
    for spec in specifiers {
        match &spec.resource.1 {
            ...
            _ => {
                env.error(
                    &spec.loc,
                    "access_of does not yet support wildcard resource specifiers; \
    ...
    for fun_id in fun_ids {
        ...
        let (param_updates, spec_used, spec_old, spec_uses_old) = {
            ...
            for (i, access) in fun_env.get_fun_param_access().iter().enumerate() {
                let (used, old) = derive_memory_from_access_specifiers(env, &access.specifiers);
            ...
            // Include access_of memory in spec_used/spec_old so that SaveMem
...
/// Validates that closures passed to functions with `access_of` declarations
/// respect the declared memory bounds. For each call site where a function-typed
/// argument is passed to a parameter with `access_of`, the argument's memory
/// footprint must be a subset of the declared `access_of` memory.
    ...
    for caller_id in fun_ids {
        let (body, caller_spec_used, caller_spec_old, caller_param_access, caller_params) = {
            ...
            (
                ...
                caller_env.get_fun_param_access().to_vec(),
        ...
        body.visit_post_order(&mut |exp| {
            if let ExpData::Call(call_id, Operation::MoveFunction(mid, fid), args) = exp {
                ...
                let (callee_param_access, callee_params) = {
                    ...
                    let access = callee_env.get_fun_param_access().to_vec();
                    if access.is_empty() {
                        return true;
                    }
                            ...
                            env.error(
                                ...
                                &format!(
                                    "function argument accesses resource `{}` \
                                     which is not declared in `access_of<{}>`",
                                    env.display(mem),
                                ...
                                env.error(
                                    ...
                                    &format!(
                                        "function argument writes resource `{}` \
                                         but `access_of<{}>` only declares reads",
                                        env.display(mem),
                                        param_name.display(env.symbol_pool())
                                    ),
                                );
...
fn compute_arg_memory(
    ...
    caller_param_access: &[FunParamAccessSpecifiers],
    ...
    match arg.as_ref() {
        ...
        // Parameter forwarding: use the caller's access_of for that parameter
        ExpData::Temporary(_, idx) => {
            ...
            // Parameter without access_of or a local — overapproximate
